...
CREDENTIALS_FERNET_KEY = os.getenv(
    "CREDENTIALS_FERNET_KEY"
)  # Base64 key for AES-256 (Fernet) encryption
...
async def verify_admin_token(
    ...
    if not ADMIN_TOKEN:
        ...
        raise HTTPException(
            status_code=401, detail="Token de infraestructura (X-Admin-Token) inválido."
        )
    ...
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(
            ...
            detail="Sesión no válida. Token JWT requerido.",
        )
...
async def update_user_status(
    user_id: str, payload: StatusUpdate, user_data=Depends(verify_admin_token)
):
    ...
    if user_data.role != "ceo":
        raise HTTPException(
            status_code=403,
            detail="Solo el CEO puede cambiar el estado de los usuarios.",
        )
...
# Modelos para documentos del paciente
...
# Modelos para registros digitales
...
async def get_chat_sessions(
    ...
    for row in rows:
        ...
        unread_params = [row["phone_number"]] + (
            [tenant_id] if has_tenant_in_cm else []
        )
        ...
        last_user_params = [row["phone_number"]] + (
            [tenant_id] if has_tenant_in_cm else []
        )
...
async def get_dashboard_stats(
    ...
    try:
        ...
        days = days_map.get(range, 99999)  # 99999 = effectively all time

        # 1. IA Conversations (Hilos únicos en el rango seleccionado)
        ia_conversations = (
            await db.pool.fetchval(
                """
            SELECT COUNT(DISTINCT from_number)
            FROM chat_messages
            WHERE tenant_id = $1 AND created_at >= CURRENT_DATE - INTERVAL '1 day' * $2
        """,
                tenant_id,
                days,
            )
...
@router.get(
    "/chat/urgencies", dependencies=[Depends(verify_admin_token)], tags=["Chat"]
)
...
async def get_clinic_settings(
    ...
    try:
        ...
        ui_lang = (
            (config.get("ui_language") or "en") if isinstance(config, dict) else "en"
        )
        return {
...
async def search_patients_by_symptoms(query: str, limit: int = 20):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error en búsqueda semántica: {str(e)}"
        )
...
async def get_patient_insurance_status(patient_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error verificando seguro: {str(e)}"
        )
...
@router.post(
    "/patients", dependencies=[Depends(verify_admin_token)], tags=["Pacientes"]
)
...
async def list_patient_documents(
    ...
    documents = await db.pool.fetch(
        """
        SELECT id, file_name, file_path, file_size, mime_type, document_type,
               uploaded_by, source, source_details, uploaded_at, created_at
    ...
    for doc in documents:
        d = dict(doc)
        d["proxy_url"] = f"/admin/patients/{id}/documents/{doc['id']}/proxy"
        out.append(d)
...
async def upload_patient_document(
    ...
    upload_dir = Path(f"uploads/demo/{tenant_id}/{id}")
...
async def delete_patient_document(
    ...
    await db.pool.execute(
        """
        DELETE FROM patient_documents WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """,
        doc_id,
        patient_id,
        tenant_id,
...
@router.get(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Servir archivo de documento",
)
async def proxy_patient_document(
    patient_id: int,
    doc_id: int,
    tenant_id: int = Depends(get_resolved_tenant_id),
):
    """
    Sirve el archivo físico de un documento.
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el documento existe y pertenece al tenant
    doc = await db.pool.fetchrow(
        """
        SELECT id, file_path, mime_type FROM patient_documents
        WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """,
        doc_id,
        patient_id,
...
# ============================================================================
# Endpoints para Registros Digitales (patient_digital_records)
# ============================================================================


@router.get(
    "/patients/{id}/digital-records",
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Listar registros digitales del paciente",
)
async def list_digital_records(
    ...
    """
    Lista todos los registros digitales de un paciente.
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el paciente existe y pertenece al tenant
    patient = await db.pool.fetchrow(
    ...
    records = await db.pool.fetch(
        """
        SELECT id, tenant_id, patient_id, professional_id, template_type, title,
               html_content, pdf_path, pdf_generated_at, source_data, generation_metadata,
               status, sent_to_email, sent_at, created_at, updated_at
    ...
    for rec in records:
        d = dict(rec)
        # Agregar proxy URL para PDF si existe
        if d.get("pdf_path"):
            d["pdf_proxy_url"] = f"/admin/patients/{id}/digital-records/{rec['id']}/pdf"
        else:
            d["pdf_proxy_url"] = None
        out.append(d)
...
@router.post(
    "/patients/{id}/digital-records/generate",
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Generar un nuevo registro digital",
)
async def generate_digital_record(
    ...
    """
    Genera un registro digital a partir de una plantilla.
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el paciente existe y pertenece al tenant
    patient = await db.pool.fetchrow(
    ...
    if request.template_type not in allowed_templates:
        raise HTTPException(status_code=400, detail="Tipo de plantilla no válido")

    # Cargar HTML fixture según template_type
    import os

    html_fixture_path = os.path.join("html_fixtures", f"{request.template_type}.html")
    if not os.path.exists(html_fixture_path):
        raise HTTPException(status_code=500, detail="Plantilla HTML no disponible")
    ...
    """,
        ...
        request.title or f"Registro {request.template_type}",
        html_content,
        {"template": request.template_type, "generated_by": user_data.user_id},
        {"fixture_used": html_fixture_path},
    )
    ...
    d = dict(record)
    if d.get("pdf_path"):
        d["pdf_proxy_url"] = f"/admin/patients/{id}/digital-records/{record_id}/pdf"
    else:
        d["pdf_proxy_url"] = None
    return d
...
@router.get(
    "/patients/{patient_id}/digital-records/{record_id}/preview",
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Obtener HTML del registro digital",
)
async def preview_digital_record(
    patient_id: int,
    record_id: uuid.UUID,
    tenant_id: int = Depends(get_resolved_tenant_id),
):
    """
    Devuelve el contenido HTML de un registro digital.
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el registro existe y pertenece al tenant
    record = await db.pool.fetchrow(
        """
        SELECT id, html_content FROM patient_digital_records
        WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """,
        record_id,
        patient_id,
...
@router.get(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Descargar PDF del registro digital",
)
async def download_digital_record_pdf(
    patient_id: int,
    record_id: uuid.UUID,
    tenant_id: int = Depends(get_resolved_tenant_id),
):
    """
    Sirve el PDF generado de un registro digital (fixture demo).
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el registro existe y pertenece al tenant
    record = await db.pool.fetchrow(
        """
        SELECT id, pdf_path FROM patient_digital_records
        WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """,
        record_id,
        patient_id,
    ...
    if not pdf_path:
        # Si no hay PDF, usar un fixture predeterminado
        pdf_path = "uploads/demo/sample-clinical-report.pdf"
...
@router.post(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Enviar registro digital por email (demo)",
)
async def email_digital_record(
    patient_id: int,
    record_id: uuid.UUID,
    ...
    """
    Simula el envío de un registro digital por email.
    En demo solo actualiza los campos, no envía realmente.
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el registro existe y pertenece al tenant
    record = await db.pool.fetchrow(
        """
        SELECT id FROM patient_digital_records
        WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """,
        record_id,
        patient_id,
...
@router.patch(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Actualizar contenido HTML del registro digital",
)
async def update_digital_record(
    ...
    """
    Actualiza campos de un registro digital (ej. html_content, status).
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el registro existe y pertenece al tenant
    record = await db.pool.fetchrow(
    ...
    allowed_fields = {"html_content", "title", "status", "template_type"}
    set_clauses = []
    params = []
    param_idx = 4  # $1,$2,$3 ya usados
    ...
    query = f"""
        ...
        WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """
    await db.pool.execute(query, record_id, patient_id, tenant_id, *params)

    return {"message": "Registro actualizado", "record_id": str(record_id)}
...
@router.delete(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Eliminar registro digital",
)
async def delete_digital_record(
    ...
    await db.pool.execute(
        """
        DELETE FROM patient_digital_records
        WHERE id = $1 AND patient_id = $2 AND tenant_id = $3
    """,
        record_id,
        patient_id,
...
@router.patch(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Pacientes"],
    summary="Actualizar anamnesis (historia médica) del paciente",
)
async def update_patient_anamnesis(
    ...
    """
    Actualiza la historia médica (anamnesis) de un paciente.
    Fusiona los campos proporcionados en el JSON existente.
    Emite evento Socket.IO PATIENT_UPDATED.
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que el paciente existe y pertenece al tenant
    patient = await db.pool.fetchrow(
        """
        SELECT id, medical_history FROM patients WHERE id = $1 AND tenant_id = $2
    """,
        id,
        tenant_id,
    )
    ...
    # Obtener medical_history actual (puede ser NULL)
    current = patient.get("medical_history") or {}
    # Fusionar updates (shallow merge, pero podemos hacer deep merge simple)
    ...
    # Emitir evento de actualización de paciente
    await emit_appointment_event(
        "PATIENT_UPDATED",
        {
            "patient_id": id,
    ...
    updated_patient = await db.pool.fetchrow(
        """
        SELECT id, first_name, last_name, phone_number, email, dni, insurance_provider,
...
@router.post(
    "/professionals", dependencies=[Depends(verify_admin_token)], tags=["Profesionales"]
)
    ...
    try:
        ...
        if current_wh:
            ...
            params = [
                ...
                payload.email,
                payload.is_active,
                json.dumps(payload.availability),
...
@router.get(
    "/patients/{id}", dependencies=[Depends(verify_admin_token)], tags=["Pacientes"]
)
...
async def add_clinical_note(
    ...
    """,
        ...
        id,
        note.content,
        json.dumps(note.odontogram_data) if note.odontogram_data else "{}",
    )
    return {"status": "ok"}
...
@router.get(
    "/appointments", dependencies=[Depends(verify_admin_token)], tags=["Turnos"]
)
...
async def update_appointment_status(id: str, payload: StatusUpdate, request: Request):
    ...
    await db.pool.execute(
        "UPDATE appointments SET status = $1 WHERE id = $2", payload.status, id
    )
    ...
    if appointment_data:
        ...
        if payload.status == "cancelled":
            ...
            await emit_appointment_event(
                "APPOINTMENT_UPDATED", dict(appointment_data), request
            )
...
@router.put(
    "/appointments/{id}", dependencies=[Depends(verify_admin_token)], tags=["Turnos"]
)
async def update_appointment(
    ...
    try:
        ...
        if full_data:
            await emit_appointment_event(
                "APPOINTMENT_UPDATED", dict(full_data), request
            )
...
@router.get(
    "/calendar/blocks", dependencies=[Depends(verify_admin_token)], tags=["Calendario"]
)
...
@router.post(
    "/sync/calendar", dependencies=[Depends(verify_admin_token)], tags=["Calendario"]
)
@router.post(
    "/calendar/sync", dependencies=[Depends(verify_admin_token)], tags=["Calendario"]
)
...
@router.get(
    ...
    dependencies=[Depends(verify_admin_token)],
    tags=["Feriados"],
    summary="Listar feriados de la clínica",
)
async def list_holidays(
    ...
    """
    Lista todos los feriados configurados para la clínica actual.
    Aislado por tenant_id (Regla de Oro).
    """
    holidays = await holiday_service.list_holidays(tenant_id)
    return holidays
...
async def create_holiday(
    ...
    """
    Crea un feriado (día de cierre) para la clínica.
    Fecha única por tenant (no puede haber duplicados).
    Aislado por tenant_id (Regla de Oro).
    """
    # Verificar que no existe ya un feriado en esa fecha para este tenant
    existing = await holiday_service.get_holiday(holiday.date, tenant_id)
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Ya existe un feriado para la fecha {holiday.date}"
        )

    row = await db.pool.fetchrow(
        """
        INSERT INTO tenant_holidays (tenant_id, date, description)
        VALUES ($1, $2, $3)
...
# ============================================
# TREATMENT PLAN BILLING - CRUD ENDPOINTS (EP-01 a EP-10)
# ============================================
...
# EP-04: PATCH /admin/treatment-plans/{plan_id}
@router.patch(
    "/treatment-plans/{plan_id}",
    tags=["Treatment Plans"],
...
async def update_plan_item(
    ...
    if item["status"] == "completed" and payload.status == ItemStatus.PENDING:
        raise HTTPException(
            status_code=422,
...
@router.post(
    ...
    tags=["Treatment Plans"],
    summary="EP-09: Registrar pago en plan de tratamiento",
    status_code=201,
)
...
# EP-10: DELETE /admin/treatment-plan-payments/{payment_id}
@router.delete(
    "/treatment-plan-payments/{payment_id}",
    tags=["Treatment Plans"],
    summary="EP-10: Eliminar pago de plan de tratamiento",
    status_code=204,
)
