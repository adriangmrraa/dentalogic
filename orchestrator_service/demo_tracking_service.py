...
class DemoTrackingService:
    async def create_or_get_lead(
        ...
        try:
            ...
            row = await db.pool.fetchrow(
                "SELECT id FROM demo_leads WHERE phone_number = $1", phone_number
            )
            if row:
                await db.pool.execute(
                    "UPDATE demo_leads SET last_seen_at = NOW() WHERE id = $1",
                    row["id"],
                )
                return row["id"]
    ...
    async def register_event(
        self, lead_id: int, event_type: str, event_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            if event_type == "whatsapp_click":
                await db.pool.execute(
                    "UPDATE demo_leads SET status = 'contacted' WHERE id = $1 AND status = 'new'",
                    lead_id,
                )
    ...
    async def track_whatsapp_message(
        self, phone_number: str, name: Optional[str] = None, channel: str = "whatsapp"
    ):
        """Registra o actualiza un lead cuando alguien habla al WhatsApp de Dentalogic."""
        try:
            row = await db.pool.fetchrow(
                "SELECT id, whatsapp_messages FROM demo_leads WHERE phone_number = $1",
                phone_number,
            )
            if row:
                await db.pool.execute(
            ...
            """,
                lead_id,
                json.dumps({"channel": channel, "name": name}),
            )
    ...
    async def track_demo_appointment(
        self, phone_number: str, appointment_type: str = "unknown"
    ):
        ...
        try:
            ...
            if row:
                ...
                """,
                    row["id"],
                    json.dumps({"type": appointment_type}),
                )
        except Exception as e:
    ...
    async def get_all_leads(self):
        ...
        rows = await db.pool.fetch(
            "SELECT * FROM demo_leads ORDER BY engagement_score DESC, last_seen_at DESC"
        )
    ...
    async def get_lead_events(self, lead_id: int):
        ...
        rows = await db.pool.fetch(
            "SELECT * FROM demo_events WHERE lead_id = $1 ORDER BY created_at DESC",
            lead_id,
        )
        return [dict(row) for row in rows]
