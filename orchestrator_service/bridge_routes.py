...
async def mark_lead_synced(lead_id: int):
    ...
    """
    try:
        await db.pool.execute("UPDATE demo_leads SET status = 'synced_to_crm' WHERE id = $1 AND status != 'synced_to_crm'", lead_id)
        return {"status": "success", "message": f"Lead {lead_id} marcado como sincronizado"}
    except Exception as e:
